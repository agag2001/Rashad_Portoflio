...
document.querySelectorAll('.nav-link').forEach(l => l.addEventListener('click', () => { if (window.innerWidth <= 1024) toggleSidebar(); }));
...
function handleReveal() { animEls.forEach(el => { if (el.getBoundingClientRect().top < window.innerHeight - 80) el.classList.add('active'); }); }
...
function animateBars() {
    ...
    if (s && s.getBoundingClientRect().top < window.innerHeight - 100) {
        barsDone = true;
        document.querySelectorAll('.skill-bar-fill').forEach((b, i) => { setTimeout(() => { b.style.width = b.dataset.width + '%'; }, i * 150); });
    }
}
...
document.getElementById('contactForm').addEventListener('submit', function(e) {
    e.preventDefault();
    const btn = this.querySelector('button[type="submit"]'), msg = document.getElementById('formSuccess');
    ...
    setTimeout(() => {
        ...
        btn.style.background = '#8B7355'; msg.style.display = 'block'; this.reset();
        setTimeout(() => {
            btn.innerHTML = '<iconify-icon icon="lucide:send" width="16"></iconify-icon> Send Message';
            btn.style.background = ''; btn.style.opacity = '1'; btn.style.pointerEvents = 'auto'; msg.style.display = 'none';
        }, 3000);
    }, 1500);
});
